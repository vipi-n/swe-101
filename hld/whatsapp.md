...
1. **User taps "New group"**, picks contacts (≤ 100), names it, hits *Create*. Client sends `createChat { participants, name }` over its open WSS.
2. **Chat Server validates** (auth, participants exist, ≤ 100, caller is in list) and generates a `chatId`.
3. **One atomic DynamoDB `transactWrite`** inserts 1 `Chat` row + N `ChatParticipant` rows — no half-created groups.
4. Server replies `{ chatId }` → sender's UI flips to the new chat.
5. For each participant **whose WS this server holds**, push `chatUpdate` so their app shows "You were added to *Family*" instantly.
6. Offline / different-server participants will see the chat on next reconnect (login reads `ChatParticipant-GSI` by `userId`). Live cross-server push comes later in [DD1](#dd1-scaling-to-billions-of-concurrent-users).
...
1. **A types "hello"**, taps send. Client renders optimistically ("sending" tick) and sends `sendMessage { chatId, message }` over its open WSS.
2. **Chat Server** looks up `ChatParticipant(chatId)` → `[A, B, C]`, generates `messageId`, stamps `serverTs` (NTP).
3. Server replies `{ messageId, serverTs, SUCCESS }` to A → tick flips to "sent".
4. For each recipient in its in-memory `userId → WS` map, push `newMessage` over that WS. (Naive: assume B, C are on this same server.)
5. B, C render the message and send `ack { messageId }`.
6. No ack in ~30s → socket marked suspect; message will be redelivered after reconnect (see [ack protocol](#the-ack-protocol--foundation-of-guaranteed-delivery)).
...
#### Step-by-step flow (A sends a message; C is offline)

**Send-time (C's phone is in a drawer):**

1. **A sends** `sendMessage` as in [3.2](#32-users-can-sendreceive-messages). Server resolves participants → `[A, B, C]`.
2. **Durability first** — server writes `Message` (TTL = 30d) + one `Inbox(clientId, messageId)` row per recipient client *before* attempting any push.
3. Server replies `SUCCESS` to A.
4. Push `newMessage` to B (online) → B acks → `DELETE Inbox(B, messageId)`.
5. C has no open WS → nothing to push. `Inbox(C, messageId)` just sits there, protected by TTL.

**Catch-up time (C opens the app hours later):**

1. C reconnects → WSS handshake → lands on some Chat Server (any one).
2. Server runs `Query Inbox WHERE recipientClientId = C` → `[m1, m2, m3, …]` ordered oldest-first.
3. For each: `GET Message(mX)` → push → wait for ack → `DELETE Inbox(C, mX)`.
4. If C never returns, rows expire after 30 days via DynamoDB TTL — no cron needed.
...
#### Step-by-step flow (A sends a photo to B)

1. **A picks a photo.** Client computes a SHA-256 hash + reads size/MIME.
2. Client calls `createAttachment { hash, sizeBytes, mimeType }` over WSS.
3. **Chat Server:** if any `Attachment` row already has this hash, reuse its `blobUrl` (dedup forwarded memes). Otherwise create `attachmentId`, write `Attachment { status: pending }`, generate a **pre-signed S3 PUT URL** (~15 min, locked to this key + size + hash).
4. Server replies `{ attachmentId, uploadUrl, downloadUrl }`.
5. **A uploads bytes directly to S3** — Chat Server stays out of the data path.
...
6. On upload success, A calls `sendMessage { chatId, message, attachmentIds }`. Server writes `Message` + `Inbox` rows as in [3.3](#33-users-can-receive-messages-sent-while-they-are-not-online-30-days) (just a reference — no bytes).
7. Push `newMessage` to B with `downloadUrl` embedded (small JSON frame).
8. **B fetches the image from the CDN**, not our servers. CDN caches at the edge → groupmates hit cache. B's UI shows placeholder → full image, then acks.
